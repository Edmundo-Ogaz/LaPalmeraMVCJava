...
var checkOK = "0123456789";
  ...
  if (formulario.insumo.value.length < 1) {
...
<table>
	<tr height="1">
		...
		<td valign="top">
			<table cellpadding="0" cellspacing="0" height="100%">
				...
				<table width="676">
				<tr>
				<td width="611"><h1>Mantenedor Ingredientes</h1></td>
				<td width="64"><button type="button" onClick="window.open('ingrediente.jsp','_self')">Atr&aacute;s</button></td>
				<td width="47"><button type="button" onClick="window.open('listar_ingrediente.jsp','_self')">Listar</button></td>
				<!--td width="50"><form action="listar_ingrediente.jsp">
	...
	<form method = "get" name = "ingrediente_modificar" onSubmit = "return validar(this)" action = "/LaPalmera/servlet/ModificaIngrediente">
  				<input name = "producto" type="hidden" value="<%=ingrediente.getCodigoProducto()%>"/>
				<table>
				<tr>
					<td width="133">Código Producto :</td>
   				  <td width="242"><input name = "producto2" type="type" value="<%=ingrediente.getCodigoProducto()%>" disabled="disabled"></td>
				</tr>
				<tr>
					<td width="133">Código Insumo :</td>
					<%ManejadorInsumo manInsumo = new ManejadorInsumo();%>
				<%Vector vecRepInsumo;
				vecRepInsumo = manInsumo.consultar();%>
				<%Enumeration enumRepInsumo = vecRepInsumo.elements();%>
			   <td><SELECT size="1" name="insumo">
				  <%while (enumRepInsumo.hasMoreElements())
			    {
			     Insumo insumo = (Insumo) enumRepInsumo.nextElement();%>
			     
				 <%if ( ingrediente.getCodigoInsumo().equals(insumo.getCodigoInsumo())){%>
				 <OPTION  selected VALUE="<%=insumo.getCodigoInsumo()%>"> <%=insumo.getNombreInsumo()%></OPTION>					   				<%}else{%>
				 <OPTION VALUE="<%=insumo.getCodigoInsumo()%>"> <%=insumo.getNombreInsumo()%></OPTION>
			    <%}%>
			    <%}%>
				</SELECT></td>
	   			    <!--td width="242"><input name = "insumo" type="type" size="40" value="<%=ingrediente.getCodigoInsumo()%>"/></td-->
				</tr>
    			<tr>
					<td>Carga :</td>
				  <td width="242"><input name = "carga" type="type" size="40" value="<%=ingrediente.getCarga()%>"/></td>
				</tr>
				<tr>
				<td>Cantidad Insumo :</td>
				<td width="242"><input name = "cantidadinsumo" type="type" size="40" value="<%=ingrediente.getCantidadInsumo()%>"/></td>
				</tr>
				<tr>
			<td></td>
	        <td></td>

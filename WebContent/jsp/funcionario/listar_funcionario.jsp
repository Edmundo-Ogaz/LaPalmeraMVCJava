	...
	<tr height="1">
		...
		<td valign="top">
			<table cellpadding="0" cellspacing="0" height="100%">
				...
				<!--td width="47"><button type="button" onClick="window.open('listar_funcionario.jsp','self')">Listar</button></td-->
				<!--td width="50"><form action="funcionario.jsp">
					<input type="submit" name="funcionario" value="Funcionario"/>
				  </form></td-->
				</tr>
				</table>
				<table border="1">
				<tr><th>Rut</th><th>Nombre</th><th>Apellido P.</th><th>Apellido M.</th><th>Cargo</th><th>Area</th></tr>
				<%ManejadorFuncionario manFuncionario = new ManejadorFuncionario();%>
				<%Vector vecRep;
				vecRep = manFuncionario.consultar();%>
				<%Enumeration enumRep = vecRep.elements();%>
				<%while (enumRep.hasMoreElements())
			    {
			    	Funcionario funcionario = (Funcionario) enumRep.nextElement();%>
			    	<tr>
			  			<td width="20">&nbsp;<%=funcionario.getRutFuncionario()%></td>
      					<td width="110">&nbsp;<%=funcionario.getNombreFuncionario()%></td>
						<td width="10">&nbsp;<%=funcionario.getApellidoPaternoFuncionario()%></td>
						<td width="10">&nbsp;<%=funcionario.getApellidoMaternoFuncionario()%></td>
						<!--td width="10"><%--=funcionario.getDireccionFuncionario()--%></td-->
						<!--td width="10"><%--=funcionario.getCodigoComuna()--%></td-->
						<!--td width="10"><%--=funcionario.getTelefonoFuncionario()--%></td-->
						<!--td width="50" align="right"><%--=funcionario.getCelularFuncionario()--%></td-->
						<!--td width="50" align="right"><%--=funcionario.getEmailFuncionario()--%></td-->
						<td width="100" align="right">&nbsp;<%=funcionario.getCodigoCargo()%></td>
						<td width="10" align="right">&nbsp;<%=funcionario.getCodigoArea()%></td>
						<!--td width="80" align="right"><%--=funcionario.getFotoProducto()--%></td-->						
						<!--td width="110" align="right"><%--=producto.getObservacionProducto()--%></td-->
						<td></td>
	  				</tr>
				<%}%>
				</table>

	...
	<tr height="1">
		...
		<td valign="top">
			<table cellpadding="0" cellspacing="0" height="100%">
				...
				<table width="676">
				<tr>
				<td width="611"><h1>Mantenedor Funcionario</h1></td>
				<td width="64"><button type="button" onClick="window.open('funcionario.jsp','_self')">Atr&aacute;s</button></td>
				<td width="47"><button type="button" onClick="window.open('listar_funcionario.jsp','_self')">Listar</button></td>
				<!--td width="50"><form action="listar_funcionario.jsp">
					<input type="submit" name="listar" value="Listar"/>
				  </form></td-->
				</tr>
				</table>
				<jsp:useBean id="funcionario" class="Funcionario" scope="session"/>
				<form method = "get" action = "consultar_operacion_funcionario.jsp" onSubmit="return validar(this)">
  				<input name="r" type="hidden" value="<%=funcionario.getRutFuncionario()%>"/>   
				<table>
				<tr>
					<td width="133">Rut Funcionario :</td>
   				  <td width="242"><input name = "rut" type="type" value="<%=funcionario.getRutFuncionario()%>" disabled="disabled" /></td>
				</tr>
				<tr>
					<td width="133">Nombre :</td>
	   			  <td width="242"><input name = "nombre" type="type" size="40" value="<%=funcionario.getNombreFuncionario()%>" disabled="disabled"/></td>
    			</tr>
    			<tr>
					<td>Apellido Paterno :</td>
				  <td width="242"><input name = "apellidopaterno" type="type" size="40" value="<%=funcionario.getApellidoPaternoFuncionario()%>" disabled="disabled"/></td>
				</tr>
				<tr>
				<td>Apellido Materno :</td>
				<td width="242"><input name = "apellidomaterno" type="type" size="40" value="<%=funcionario.getApellidoMaternoFuncionario()%>" disabled="disabled"/></td>
				</tr>
				
			<tr>
				  <td>C&oacute;digo Cargo :</td>
		   ...
		   <%}%>
				  ...
				  <!--td width="242"><input name = "cargo" type="type" size="40" value="<%=funcionario.getCodigoCargo()%>" disabled="disabled"/></td-->

			</tr>
			<tr>
				  <td>C&oacute;digo Area :</td>
				  
			<%ManejadorArea manArea = new ManejadorArea();%>
			<%Vector vecArea;
			vecArea = manArea.consultar();%>
			<%Enumeration enumArea = vecArea.elements();%>
			   <td><SELECT size="1" name="area" disabled="disabled">
			  <%while (enumArea.hasMoreElements())
    			{
		     Area area = (Area) enumArea.nextElement();%>
			  <% if (funcionario.getCodigoArea().equals(area.getCodigoArea())){%>
			  <OPTION selected="selected" VALUE="<%=area.getCodigoArea()%>"> <%=area.getNombreArea()%></OPTION>
			 <%}else{%>  
		     <OPTION VALUE="<%=area.getCodigoArea()%>"> <%=area.getNombreArea()%></OPTION>
		   <%}%>
		   <%}%>
			</SELECT></td>	  
				  <!--td width="242"><input name = "area" type="type" size="40" value="<%=funcionario.getCodigoArea()%>" disabled="disabled"/></td-->

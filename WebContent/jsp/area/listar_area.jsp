	...
	<tr height="1">
		...
		<td valign="top">
			<table cellpadding="0" cellspacing="0" height="100%">
				...
				<!--td width="47"><button type="button" onClick="window.open('listar_area.jsp','self')">Listar</button></td-->
				<!--td width="50"><form action="area.jsp">
					<input type="submit" name="area" value="Area"/>
				  </form></td-->
				</tr>
				</table>
				<table border="1">
				<tr><th>C&oacute;digo Area</th><th>Nombre</th></tr>
				<%ManejadorArea manArea = new ManejadorArea();%>
				<%Vector vecRep;
				vecRep = manArea.consultar();%>
				<%Enumeration enumRep = vecRep.elements();%>
				<%while (enumRep.hasMoreElements())
			    {
			    	Area area = (Area) enumRep.nextElement();%>
			    	<tr>
			  			<td width="20" align="right">&nbsp;<%=area.getCodigoArea()%></td>
      					<td width="110" align="left">&nbsp;<%=area.getNombreArea()%></td>
						<td></td>
	  				</tr>
				<%}%>
				</table>

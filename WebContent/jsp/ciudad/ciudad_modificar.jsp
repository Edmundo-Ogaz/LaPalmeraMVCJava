...
var checkOK = "0123456789";
  ...
  if (!allValid) {
    alert("Escriba sólo números en el campo \"Código Ciudad\".");
    formulario.codigo.focus();
    return (false);
  }    
  ...
  if (formulario.region.value.length < 1) {
...
<table>
	<tr height="1">
		...
		<td valign="top">
			<table cellpadding="0" cellspacing="0" height="100%">
				...
				<table width="676">
				<tr>
				<td width="611"><h1>Mantenedor Ciudad</h1></td>
				<td width="64"><button type="button" onClick="window.open('ciudad.jsp','_self')">Atr&aacute;s</button></td>
				<td width="47"><button type="button" onClick="window.open('listar_ciudad.jsp','_self')">Listar</button></td>
				<!--td width="50"><form action="listar_ciudad.jsp">
	...
	<form method = "get" name = "ciudad_modificar" onSubmit = "return validar(this)" action = "/LaPalmera/servlet/ModificaCiudad">
  				<input name="codigo" type="hidden" value="<%=ciudad.getCodigoCiudad()%>"/>
  				<table>
				<tr>
					<td width="133">Cdigo Ciudad :</td>
   				  <td width="242"><input name = "codigo2" type="type" value="<%=ciudad.getCodigoCiudad()%>" disabled="disabled"/></td>
				</tr>
				<tr>
					<td width="133">Nombre :</td>
	   			    <td width="242"><input name = "nombre" type="type" size="40" value="<%=ciudad.getNombreCiudad()%>"/></td>
				</tr>
    			<tr>
					<td>C&oacute;digo Regin :</td>
					<%ManejadorRegion manRegion = new ManejadorRegion();%>
				<%Vector vecRegion;
				vecRegion = manRegion.consultar();%>
				<%Enumeration enumRegion = vecRegion.elements();%>
			   <td><SELECT size="1" name="region">
				  <%while (enumRegion.hasMoreElements())
			    {
			     Region region = (Region) enumRegion.nextElement();%>
			     
				 <%if ( ciudad.getCodigoRegion().equals(region.getCodigoRegion())){%>
				 <OPTION  selected VALUE="<%=region.getCodigoRegion()%>"> <%=region.getNombreRegion()%></OPTION>					   				<%}else{%>
				 <OPTION VALUE="<%=region.getCodigoRegion()%>"> <%=region.getNombreRegion()%></OPTION>
			    <%}%>
			    <%}%>
				</SELECT></td>
				  <!--td width="242"><input name = "region" type="type" size="40" value="<%=ciudad.getCodigoRegion()%>"/></td-->
				</tr>
			<tr>
			<td></td>
	        <td></td>

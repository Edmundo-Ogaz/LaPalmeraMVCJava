	...
	<tr height="1">
		...
		<td valign="top">
			<table cellpadding="0" cellspacing="0" height="100%">
				...
				<form method = "get" action = "consultar_operacion_cliente.jsp" onSubmit="return validar(this)">
  				<input name = "r" type="hidden" value="<%=cliente.getRutCliente()%>"/>
  				<table>
				<tr>
					<td width="133">Rut Cliente :</td>
   				  <td width="242"><input name = "rut" type="type" value="<%=cliente.getRutCliente()%>" disabled="disabled" /></td>
				</tr>
				<tr>
					<td width="133">Nombre :</td>
	   			  <td width="242"><input name = "nombre" type="type" size="40" value="<%=cliente.getNombreCliente()%>" disabled="disabled"/></td>
    			</tr>
    			<tr>
					<td>Dirección :</td>
				  <td width="242"><input name = "direccion" type="type" size="40" value="<%=cliente.getDireccionCliente()%>" disabled="disabled"/></td>
				</tr>
				<tr>
				<td>Comuna :</td>
			   ...
			   <td><SELECT size="1" name="comuna" disabled="disabled">
				  <%while (enumComuna.hasMoreElements())
			    {
			     Comuna comuna = (Comuna) enumComuna.nextElement();%>
				 <%if (cliente.getCodigoComuna().equals(comuna.getCodigoComuna())){%>
				 <OPTION selected="selected" VALUE="<%=comuna.getCodigoComuna()%>"> <%=comuna.getNombreComuna()%></OPTION>
				 <%}else{%>
			     <OPTION VALUE="<%=comuna.getCodigoComuna()%>"> <%=comuna.getNombreComuna()%></OPTION>
			   <%}%>
			   <%}%>
				</SELECT></td>
				<!--td width="242"><input name = "comuna" type="type" size="40" value="<%=cliente.getCodigoComuna()%>" disabled="disabled"/></td-->
				</tr>
				<tr>
				  <td>Fecha Nacimiento :</td>
				  <td width="242"><input name = "fechanacimiento" type="type" size="40" value="<%=cliente.getFechaNacimientoCliente()%>" disabled="disabled"/></td>

			</tr>
			<tr>
				  <td>Tel&eacute;fono :</td>
				  <td width="242"><input name = "telefono" type="type" size="40" value="<%=cliente.getTelefonoCliente()%>" disabled="disabled"/></td>

			</tr>
			<tr>
				  <td>E-Mail :</td>
				  <td width="242"><input name = "email" type="type" size="40" value="<%=cliente.getEmailCliente()%>" disabled="disabled"/></td>

			</tr>
			<tr>
				  <td>P&aacute;gina Web :</td>
				  <td width="242"><input name = "paginaweb" type="type" size="40" value="<%=cliente.getPaginaWebCliente()%>" disabled="disabled"/></td>
			...
			<tr>
			   ...
			   <td><SELECT size="1" name="tipocliente" disabled="disabled">
				  <%while (enumTipoCliente.hasMoreElements())
			    {
			     TipoCliente tipoCliente = (TipoCliente) enumTipoCliente.nextElement();%>
				 <%if (cliente.getCodigoTipoCliente().equals(tipoCliente.getCodigoTipoCliente())){%>
				 <OPTION selected="selected" VALUE="<%=tipoCliente.getCodigoTipoCliente()%>"> <%=tipoCliente.getNombreTipoCliente()%></OPTION>
				  <%}else{%>
			     <OPTION VALUE="<%=tipoCliente.getCodigoTipoCliente()%>"> <%=tipoCliente.getNombreTipoCliente()%></OPTION>
			   <%}%>
			    <%}%>
				</SELECT></td>
				  <!--td width="242"><input name = "tipocliente" type="type" size="40" value="<%=cliente.getCodigoTipoCliente()%>" disabled="disabled"/></td-->

			</tr>
			<tr>
				  <td>Observaci&oacute;n :</td>
				  <td width="242"><input name = "observacion" type="type" size="40" value="<%=cliente.getObservacionCliente()%>" disabled="disabled"/></td>

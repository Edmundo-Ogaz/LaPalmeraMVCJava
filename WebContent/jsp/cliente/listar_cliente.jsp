	...
	<tr height="1">
		...
		<td valign="top">
			<table cellpadding="0" cellspacing="0" height="100%">
				...
				<!--td width="47"><button type="button" onClick="window.open('listar_cliente.jsp','self')">Listar</button></td-->
				<!--td width="50"><form action="cliente.jsp">
					<input type="submit" name="cliente" value="Cliente"/>
				  </form></td-->
				</tr>
				</table>
				<table border="1">
				<tr><th>Rut</th><th>Nombre</th><th>Direcci&oacute;n</th><th>Comuna</th><th>Fecha Nacimiento</th><th>Teléfono</th><th>E-mail</th><th>Página Web</th><th>Tipo Cliente</th><th>Observación</th></tr>
				<%ManejadorCliente manCliente = new ManejadorCliente();%>
				<%Vector vecRep;
				vecRep = manCliente.consultar();%>
				<%Enumeration enumRep = vecRep.elements();%>
				<%while (enumRep.hasMoreElements())
			    {
			    	Cliente cliente = (Cliente) enumRep.nextElement();%>
			    	<tr>
			  			<td width="20" align="right">&nbsp;<%=cliente.getRutCliente()%></td>
      					<td width="110" align="left">&nbsp;<%=cliente.getNombreCliente()%></td>
						<td width="10" align="left">&nbsp;<%=cliente.getDireccionCliente()%></td>
						<td width="10" align="right">&nbsp;<%=cliente.getCodigoComuna()%></td>
						<td width="10" align="right">&nbsp;<%=cliente.getFechaNacimientoCliente()%></td>
						<td width="10" align="right">&nbsp;<%=cliente.getTelefonoCliente()%></td>
						<td width="10" align="left">&nbsp;<%=cliente.getEmailCliente()%></td>
						<td width="50" align="left">&nbsp;<%=cliente.getPaginaWebCliente()%></td>
						<td width="50" align="right">&nbsp;<%=cliente.getCodigoTipoCliente()%></td>
						<td width="100" align="left">&nbsp;<%=cliente.getObservacionCliente()%></td>
						<td></td>
	  				</tr>
				<%}%>
				</table>

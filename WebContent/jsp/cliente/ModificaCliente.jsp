...
comuna = cliente.getCodigoComuna();
fechanacimiento = cliente.getFechaNacimientoCliente();
telefono = cliente.getTelefonoCliente();
email = cliente.getEmailCliente();
paginaweb = cliente.getPaginaWebCliente();
...
<body>
<form name = formulario action = "/LaPalmera/servlet/ModificaCliente" onSubmit="return validar(this)" target="_self">
...
<td>
<input name = "rut2" type="text" value="<%=rut%>" disabled="disabled"/>
</td>
</tr>
<tr>
<td>
...
<td>
<input name = "direccion" type="type" size="40" value="<%=direccion%>"/>
</td>
</tr>
<tr>
<td>
...
while (itComuna.hasNext())
    {
Comuna comuna2= (Comuna) itComuna.next();
if (comuna.equals(comuna2.getCodigoComuna())){%>
<OPTION selected="selected" VALUE=<%=comuna2.getCodigoComuna()%>> <%=comuna2.getNombreComuna()%></OPTION>
<%}else{%>
<OPTION VALUE=<%=comuna2.getCodigoComuna()%>> <%=comuna2.getNombreComuna()%></OPTION>
<%}}%>
</SELECT>
</td>
</tr>
...
<td><input type="text" name="fechanacimiento" id="campo_fecha" value="<%=fechanacimiento%>"/>
...
<!--td>
<input name = "fechanacimiento" type="type" size="40" value="<%=fechanacimiento%>"/>
</td-->
</tr>
<tr>
<td>
...
<td>
<input name = "telefono" type="type" size="40" value="<%=telefono%>"/>
</td>
</tr>
<tr>
<td>
...
<td>
<input name = "email" type="type" size="40" value="<%=email%>"/>
</td>
</tr>
<tr>
<td>
...
<td>
<input name = "paginaweb" type="type" size="40" value="<%=paginaweb%>"/>
</td>
</tr>
<tr>
<td>Tipo Cliente
...
if (tipocliente.equals(tipoCliente.getCodigoTipoCliente())){%>
...
<td>
<input name = "observacion" type="type" size="40" value="<%=observacion%>"/>
</td>
</tr>
</table>
<input type=submit name=Enviar value = 'Enviar'>
<button type="button" onClick="window.open('/LaPalmera/jsp/UsoSistema.jsp?destino=ListaCliente.jsp&titulo=Listado Cliente&filrut=<%=filrut%>&filnombre=<%=filnombre%>&fildireccion=<%=fildireccion%>&filcomuna=<%=filcomuna%>&filtelefono=<%=filtelefono%>&filfechanacimiento=<%=filfechanacimiento%>&filemail=<%=filemail%>&filpaginaweb=<%=filpaginaweb%>&filtipocliente=<%=filtipocliente%>&filobservacion=<%=filobservacion%>','_self')">Volver</button>

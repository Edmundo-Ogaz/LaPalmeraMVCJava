...
var checkOK = "0123456789";
  ...
  if (formulario.bodega.value.length < 1) {
...
<table>
	<tr height="1">
		...
		<td valign="top">
			<table cellpadding="0" cellspacing="0" height="100%">
				...
				<table width="676">
				<tr>
				<td width="611"><h1>Mantenedor Ubicación</h1></td>
				<td width="64"><button type="button" onClick="window.open('ubicacion.jsp','_self')">Atr&aacute;s</button></td>
				<td width="47"><button type="button" onClick="window.open('listar_ubicacion.jsp','self')">Listar</button></td>
				<!--td width="50"><form action="listar_ubicacion.jsp">
	...
	<form method = "get" name = "ubicacion_modificar" onSubmit = "return validar(this)" action = "/LaPalmera/servlet/ModificaUbicacion">
  				<input name = "producto" type="hidden" value="<%=ubicacion.getCodigoProducto()%>"/>
				<table>
				<tr>
					<td width="133">Código Producto :</td>
   				  <td width="242"><input name = "producto" type="type" value="<%=ubicacion.getCodigoProducto()%>" disabled="disabled"/></td>
				</tr>
				<tr>
					<td width="133">Código Bodega :</td>
							<%ManejadorBodega manBodega = new ManejadorBodega();%>
			<%Vector vecBodega;
			vecBodega = manBodega.consultar();%>
			<%Enumeration enumBodega = vecBodega.elements();%>
			   <td><SELECT size="1" name="bodega">
			  <%while (enumBodega.hasMoreElements())
    			{
		     Bodega bodega = (Bodega) enumBodega.nextElement();%>
		     <% if (ubicacion.getCodigoBodega().equals(bodega.getCodigoBodega())){%>
			 <OPTION selected="selected" VALUE="<%=bodega.getCodigoBodega()%>"> <%=bodega.getNombreBodega()%></OPTION>
			 <%}else{%>
			 <OPTION VALUE="<%=bodega.getCodigoBodega()%>"> <%=bodega.getNombreBodega()%></OPTION>
		   <%}%>
		   <%}%>
			</SELECT></td>
	   			  <!--td width="242"><input name = "bodega" type="type" size="40" value="<%=ubicacion.getCodigoBodega()%>"/></td-->
    			</tr>
    			<tr>
					<td>Tipo :</td>
				  <td width="242"><input name = "tipo" type="type" size="40" value="<%=ubicacion.getTipoEntidad()%>"/></td>
				</tr>
				<tr>
				<td>Fecha Ingreso :</td>
				<td width="242"><input name = "fechaingreso" type="type" size="40" value="<%=ubicacion.getFechaIngreso()%>"/></td>
				</tr>
				<tr>
				  <td>FechaVencimiento :</td>
				  <td width="242"><input name = "fechavencimiento" type="type" size="40" value="<%=ubicacion.getFechaVencimiento()%>"/></td>

			</tr>
			<tr>
				  <td>Cantidad :</td>
				  <td width="242"><input name = "cantidad" type="type" size="40" value="<%=ubicacion.getCantidad()%>"/></td>

			</tr>
			<tr>
				  <td>Observación :</td>
				  <td width="242"><input name = "observacion" type="type" size="40" value="<%=ubicacion.getObservacion()%>"/></td>
	  ...
	  <td><input name="cancelar" value="Cancelar" type="submit"></td>
    </tr>
  </table>
		</form>

	...
	<tr height="1">
		...
		<td valign="top">
			<table cellpadding="0" cellspacing="0" height="100%">
				...
				<table width="676">
				<tr>
				<td width="611"><h1>Mantenedor Insumo</h1></td>
				<td width="64"><button type="button" onClick="window.open('insumo.jsp','_self')">Atr&aacute;s</button></td>
				<td width="47"><button type="button" onClick="window.open('listar_insumo.jsp','_self')">Listar</button></td>
				<!--td width="50"><form action="listar_insumo.jsp">
					<input type="submit" name="listar" value="Listar"/>
				  </form></td-->
				</tr>
				</table>
				<jsp:useBean id="insumo" class="Insumo" scope="session"/>
				<form method = "get" action = "consultar_operacion_insumo.jsp" onSubmit = "return validar(this)">
  				<input name = "cod" type="hidden" value="<%=insumo.getCodigoInsumo()%>"/>
				<table>
				<tr>
					<td width="133">Código Insumo :</td>
   				  <td width="242"><input name = "codigo" type="type" value="<%=insumo.getCodigoInsumo()%>" disabled="disabled" /></td>
				</tr>
				<tr>
					<td width="133">Nombre :</td>
	   			  <td width="242"><input name = "nombre" type="type" size="40" value="<%=insumo.getNombreInsumo()%>" disabled="disabled"/></td>
    			</tr>
    			<tr>
					<td>Codigo Unidad de Medida :</td>
			...
			<%Enumeration enumRep4 = vecRep4.elements();%>
			   <td><SELECT size="1" name="unidadmedida" disabled="disabled">
			  <%while (enumRep4.hasMoreElements())
    			{
		     UnidadMedida unidadMedida = (UnidadMedida) enumRep4.nextElement();%>
			 <%if ( insumo.getCodigoUnidadMedida().equals(unidadMedida.getCodigoUnidadMedida())){%>
		   ...
		   <%}%>
				  ...
				  <!--td width="242"><input name = "unidadmedida" type="type" size="40" value="<%=insumo.getCodigoUnidadMedida()%>" disabled="disabled"/></td-->
				</tr>
				<tr>
				<td>Stock Critico :</td>
				<td width="242"><input name = "stockcritico" type="type" size="40" value="<%=insumo.getStockCriticoInsumo()%>" disabled="disabled"/></td>
				</tr>
				<tr>
				  <td>Observación :</td>
				  <td colspan="3"><TEXTAREA name="observacion" rows = 5 cols = 60 disabled="disabled"><%=insumo.getObservacionInsumo()%></TEXTAREA></td>
				  <!--td width="242"><input name = "observacion" type="type" size="40" value="<%=insumo.getObservacionInsumo()%>" disabled="disabled"/></td-->

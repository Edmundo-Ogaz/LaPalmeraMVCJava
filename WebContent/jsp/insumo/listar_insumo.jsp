	...
	<tr height="1">
		...
		<td valign="top">
			<table cellpadding="0" cellspacing="0" height="100%">
				...
				<!--td width="47"><button type="button" onClick="window.open('listar_unidadmedida.jsp','self')">Listar</butt></td-->
				<!--td width="50"><form action="insumo.jsp">
					<input type="submit" name="insumo" value="Insumo"/>
				  </form></td-->
				</tr>
				</table>
				<table border="1">
				<tr><th>Código</th><th>Nombre</th><th>Unidad Medida</th><th>Stock Critico</th><th>Observación</th></tr>
				<%ManejadorInsumo manInsumo = new ManejadorInsumo();%>
				<%Vector vecRep;
				vecRep = manInsumo.consultar();%>
				<%Enumeration enumRep = vecRep.elements();%>
				<%while (enumRep.hasMoreElements())
			    {
			    	Insumo insumo = (Insumo) enumRep.nextElement();%>
			    	<tr>
			  			<td width="20" align="right">&nbsp;<%=insumo.getCodigoInsumo()%></td>
      					<td width="110" align="left">&nbsp;<%=insumo.getNombreInsumo()%></td>
						<td width="10" align="right">&nbsp;<%=insumo.getCodigoUnidadMedida()%></td>
						<td width="10" align="right">&nbsp;<%=insumo.getStockCriticoInsumo()%></td>
						<td width="10" align="left">&nbsp;<%=insumo.getObservacionInsumo()%></td>
						<!--td width="10"><%--=insumo.getCodigoUnidadMedida()--%></td-->
						<!--td width="10"><%--=producto.getStockCriticoProducto()--%></td-->
						<!--td width="50" align="right"><%--=producto.getValorCostoProducto()--%></td-->
						<!--td width="50" align="right"><%--=producto.getValorVentaProducto()--%></td-->
						<!--td width="100" align="right"><%--=producto.getPreparacionProducto()--%></td-->
						<!--td width="10" align="right"><%--=producto.getProductoSemielaborado()--%></td-->
						<!--td width="80" align="right"><%--=producto.getFotoProducto()--%></td-->						

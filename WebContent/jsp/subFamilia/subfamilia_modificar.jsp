...
function validar(formulario) {
  if (formulario.codigo.value.length < 1) {
    alert("Escriba el código de la Subfamilia.");
...
var checkOK = "0123456789";
  ...
  if (formulario.nombre.value.length < 1) {
    alert("Escriba el nombre de la Subfamilia.");
...
<table>
	<tr height="1">
		...
		<td valign="top">
			<table cellpadding="0" cellspacing="0" height="100%">
				...
				<table width="676">
				<tr>
				<td width="611"><h1>Mantenedor Subfamilia</h1></td>
				<td width="64"><button type="button" onClick="window.open('subfamilia.jsp','_self')">Atr&aacute;s</button></td>
				<td width="47"><button type="button" onClick="window.open('listar_subfamilia.jsp','_self')">Listar</button></td>
				<!--td width="50"><form action="listar_subfamilia.jsp">
	...
	<form method = "get" name = "subfamilia_modificar" onSubmit = "return validar(this)" action = "/LaPalmera/servlet/ModificaSubfamilia">
  				<input name = "codigo" type="hidden" value="<%=subfamilia.getCodigoSubfamilia()%>"/>
				<table>
				<tr>
					<td width="133">Código Subfamilia :</td>
   				  <td width="242"><input name = "codigo2" type="type" value="<%=subfamilia.getCodigoSubfamilia()%>" disabled="disabled"/></td>
				</tr>
				<tr>
					<td width="133">Nombre :</td>
	   			    <td width="242"><input name = "nombre" type="type" size="40" value="<%=subfamilia.getNombreSubfamilia()%>"/></td>
				</tr>
    			<tr>
			...
			<%Iterator itFamilia = arrayFamilia.iterator();%>
		     ...
		     <%if (subfamilia.getCodigoFamilia().equals(familia.getCodigoFamilia())){%>
			 <OPTION selected="selected" VALUE="<%=familia.getCodigoFamilia()%>"> <%=familia.getNombreFamilia()%></OPTION>
			 <%}else{%>
			 <OPTION VALUE="<%=familia.getCodigoFamilia()%>"> <%=familia.getNombreFamilia()%></OPTION>
		   <%}%>
		   <%}%>
			</SELECT></td>
				  <!--td width="242"><input name = "familia" type="type" size="40" value="<%=subfamilia.getCodigoFamilia()%>"/></td-->
				</tr>
				<tr>
			<td></td>
	        <td></td>

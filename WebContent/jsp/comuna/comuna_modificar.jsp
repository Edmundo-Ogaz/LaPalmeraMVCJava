...
var checkOK = "0123456789";
  ...
  if (!allValid) {
    alert("Escriba sólo números en el campo \"Código Comuna\".");
    formulario.codigo.focus();
    return (false);
  }    
	...
	<tr height="1">
		...
		<td valign="top">
			<table cellpadding="0" cellspacing="0" height="100%">
				...
				<table width="676">
				<tr>
				<td width="611"><h1>Mantenedor Comuna</h1></td>
				<td width="64"><button type="button" onClick="window.open('comuna.jsp','_self')">Atr&aacute;s</button></td>
				<td width="47"><button type="button" onClick="window.open('listar_comuna.jsp','_self')">Listar</button></td>
				<!--td width="50"><form action="listar_comuna.jsp">
					<input type="submit" name="listar" value="Listar"/>
				  </form></td-->
				</tr>
				</table>
				
				<jsp:useBean id="comuna" class="Comuna" scope="session"/>
	<form method = "get" name = "comuna_modificar" onSubmit = "return validar(this)" action = "/LaPalmera/servlet/ModificaComuna">
  				<input name="codigo" type="hidden" value="<%=comuna.getCodigoComuna()%>"/>
  				<table>
				<tr>
					<td width="133">C&oacute;digo Comuna :</td>
   				  <td width="242"><input name = "codigo2" type="type" value="<%=comuna.getCodigoComuna()%>" disabled="disabled"/></td>
				</tr>
				<tr>
					<td width="133">Nombre :</td>
	   			    <td width="242"><input name = "nombre" type="type" size="40" value="<%=comuna.getNombreComuna()%>"/></td>
				</tr>
    			<tr>
					<td>Ciudad :</td>
					<%ManejadorCiudad manCiudad = new ManejadorCiudad();%>
				<%Vector vecCiudad;
				vecCiudad = manCiudad.consultar();%>
				<%Enumeration enumCiudad = vecCiudad.elements();%>
			   <td><SELECT size="1" name="ciudad">
				  <%while (enumCiudad.hasMoreElements())
			    {
			     Ciudad ciudad = (Ciudad) enumCiudad.nextElement();%>
			     <%if (comuna.getCodigoCiudad().equals(ciudad.getCodigoCiudad())){%>
				 <OPTION  selected="selected" VALUE="<%=ciudad.getCodigoCiudad()%>"> <%=ciudad.getNombreCiudad()%></OPTION>
				 <%}else{%>

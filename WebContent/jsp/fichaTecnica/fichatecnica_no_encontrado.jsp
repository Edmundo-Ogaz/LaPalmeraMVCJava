...
function validar(formulario) {
  if (formulario.producto.value.length < 1) {
    alert("Escriba el código del Producto.");
    formulario.producto.focus();
    return (false);
  }
var checkOK = "0123456789";
  var checkStr = formulario.producto.value;
  var allValid = true;
  var decPoints = 0;
  var allNum = "";
  ...
  if (formulario.conceptotecnico.value.length < 1) {
    alert("Escriba el Código del Concepto Tecnico.");
    formulario.conceptotecnico.focus();
    return (false);
  }
  var checkOK = "0123456789";
  var checkStr = formulario.conceptotecnico.value;
  var allValid = true;
  var decPoints = 0;
  var allNum = "";
  for (i = 0; i < checkStr.length; i++) {
  ...
  if (formulario.descripcion.value.length < 1) {
    alert("Escriba la descripcion del Producto.");
	...
	<tr height="1">
		...
		<td valign="top">
			<table cellpadding="0" cellspacing="0" height="100%">
				...
				<table width="676">
				<tr>
				<td width="611"><h1>Mantenedor Ficha Técnica</h1></td>
				<td width="64"><button type="button" onClick="window.open('fichatecnica.jsp','_self')">Atr&aacute;s</button></td>
				<td width="47"><button type="button" onClick="window.open('listar_fichatecnica.jsp','_self')">Listar</button></td>
				<!--td width="50"><form action="listar_fichatecnica.jsp">
					<input type="submit" name="listar" value="Listar"/>
				  </form></td-->
				</tr>
				</table>
			<jsp:useBean id="fichaTecnica" class="FichaTecnica" scope="session"/>
			<form method = "get" name = "ingreso_Formulario" onSubmit = "return validar(this)" action = "/LaPalmera/servlet/GrabaFichaTecnica">
  				<input name = "producto" type="hidden" value="<%=fichaTecnica.getCodigoProducto()%>"/>
				<table width="604">
				<tr>
			  			<td width="213">C&oacute;digo Producto :</td>
				  <td width="250"><input name = "producto2" type="type" value="<%=fichaTecnica.getCodigoProducto()%>" disabled="disabled"/></td>
				<tr>
			      <td width="213">Código Concepto Técnico :</td>
				  <%ManejadorConceptoTecnico manConceptoTecnico = new ManejadorConceptoTecnico();%>
				<%Vector vecConceptoTecnico;
				vecConceptoTecnico = manConceptoTecnico.consultar();%>
				<%Enumeration enumConceptoTecnico = vecConceptoTecnico.elements();%>
			   <td><SELECT size="1" name="conceptotecnico">
				  <%while (enumConceptoTecnico.hasMoreElements())
			    {
			     ConceptoTecnico conceptoTecnico = (ConceptoTecnico) enumConceptoTecnico.nextElement();%>
			     
				 <%if ( fichaTecnica.getCodigoConceptoTecnico().equals(conceptoTecnico.getCodigoConceptoTecnico())){%>
				 <OPTION  selected VALUE="<%=conceptoTecnico.getCodigoConceptoTecnico()%>"> <%=conceptoTecnico.getNombreConceptoTecnico()%></OPTION>					   				<%}else{%>
				 <OPTION VALUE="<%=conceptoTecnico.getCodigoConceptoTecnico()%>"> <%=conceptoTecnico.getNombreConceptoTecnico()%></OPTION>
			    <%}%>
			    <%}%>
				</SELECT></td>
	   			<!--td width="250"><input name = "conceptotecnico" type="type" size="40"/></td-->
    			</tr>
    		<tr>
			<td>Descripción</td>
			   <td width="250"><input name = "descripcion" type="type" size="40"/></td>
			</tr>
			<tr>
			<td></td>
	        <td></td>
	  <td width="55"><input name="grabar" value="Grabar" type="submit"></td>
	  <td width="66"><input name="cancelar" value="Cancelar" type="button"></td>
    </tr>
  </table>
</form>
	</td>
